"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.imageUploadService = void 0;
var fs_1 = __importDefault(require("fs"));
var errors_1 = require("../errors");
var config_1 = require("../config");
/**
 * Class: ImageUploadService
 * -------------------------
 * This class provides methods for uploading and deleting images using Cloudinary.
 */
var ImageUploadService = /** @class */ (function () {
    function ImageUploadService() {
        this.generateRandom10DigitNumber = function () {
            return Math.floor(1000000000 + Math.random() * 9000000000).toString();
        };
    }
    /**
     * Method: imageUpload
     * -------------------
     * Uploads an image to Cloudinary.
     *
     * @param folderName - A string representing the destination folder for the uploaded image.
     * @param file - The uploaded file to be processed.
     * @returns A Promise that resolves to the uploaded image information.
     */
    ImageUploadService.prototype.imageUpload = function (folderName, file) {
        return __awaiter(this, void 0, void 0, function () {
            var imageType, fileNameWithoutExtension, fileName, tempFilePath, fileInfo, err_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        imageType = file.mimetype.split("/")[0];
                        // Check if the file is an image
                        if (imageType !== "image") {
                            throw new errors_1.BadRequestError("Enter valid image type");
                        }
                        fileNameWithoutExtension = file.name.substring(0, file.name.lastIndexOf("."));
                        fileName = "".concat(fileNameWithoutExtension.toLocaleUpperCase(), "-").concat(this.generateRandom10DigitNumber());
                        tempFilePath = file.tempFilePath;
                        return [4 /*yield*/, config_1.cloudinary.uploader.upload(tempFilePath, {
                                timeout: 12000000,
                                resource_type: "auto",
                                public_id: "Nithub/".concat(folderName, "/").concat(fileName),
                                chunk_size: 8000001,
                            })
                            // Delete the temporary file generated when uploading the image
                        ];
                    case 1:
                        fileInfo = _a.sent();
                        // Delete the temporary file generated when uploading the image
                        fs_1.default.unlinkSync(tempFilePath);
                        // Remove the API key from the fileInfo object
                        delete fileInfo.api_key;
                        return [2 /*return*/, fileInfo];
                    case 2:
                        err_1 = _a.sent();
                        if (err_1 instanceof errors_1.ApiError) {
                            throw err_1;
                        }
                        throw new Error(err_1 === null || err_1 === void 0 ? void 0 : err_1.message);
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    /**
     * Method: imageUpload
     * -------------------
     * Uploads Images in Bulk to Cloudinary.
     *
     * @param folderName - A string representing the destination folder for the uploaded image.
     * @param files - An Array of uploaded files to be processed.
     * @returns A Promise that resolves to the uploaded images information.
     */
    ImageUploadService.prototype.bulkUpload = function (folderName, files) {
        return __awaiter(this, void 0, void 0, function () {
            var response_1, err_2;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        response_1 = [];
                        return [4 /*yield*/, Promise.all(files.map(function (file) { return __awaiter(_this, void 0, void 0, function () {
                                var fileInfo;
                                return __generator(this, function (_a) {
                                    switch (_a.label) {
                                        case 0: return [4 /*yield*/, this.imageUpload(folderName, file)];
                                        case 1:
                                            fileInfo = _a.sent();
                                            response_1.push(fileInfo);
                                            return [2 /*return*/];
                                    }
                                });
                            }); }))];
                    case 1:
                        _a.sent();
                        return [2 /*return*/, response_1];
                    case 2:
                        err_2 = _a.sent();
                        console.log(err_2);
                        if (err_2 instanceof errors_1.ApiError) {
                            throw err_2;
                        }
                        throw new Error(err_2.error.message);
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    /**
     * Method: deleteImage
     * --------------------
     * Deletes an image from Cloudinary using its public ID.
     *
     * @param filePublicId - An array of public IDs of the images to be deleted.
     * @returns A Promise that resolves to the result of the image deletion operation.
     */
    ImageUploadService.prototype.deleteImage = function (filePublicId) {
        return __awaiter(this, void 0, void 0, function () {
            var deletedFile, err_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, config_1.cloudinary.api.delete_resources(filePublicId, {
                                resource_type: "image",
                            })];
                    case 1:
                        deletedFile = _a.sent();
                        return [2 /*return*/, deletedFile];
                    case 2:
                        err_3 = _a.sent();
                        throw new Error(err_3.error.message);
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    return ImageUploadService;
}());
exports.imageUploadService = new ImageUploadService();
